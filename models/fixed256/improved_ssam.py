...
from iharm.data.compose import ComposeDataset, MyPreviousDataset
...
def init_model(cfg):
    ...
    model = SSAMImageHarmonization(
        ...
        attend_from=2, batchnorm_from=2, use_attn=True
    )
...
def train(model, cfg, model_cfg, start_epoch=0):
    ...
    pre_images = [torch.randn((2, 3, 256, 256)).cuda()]
    pre_masks = [torch.randn((2,1,256,256)).cuda()]
    previous= {'images': pre_images, 'masks': pre_masks}
    dic = model(img, mask, previous=previous)
    output = dic['images']
    print(output.shape, 'yes')
    exit()
    '''
    ...
    trainset = MyPreviousDataset(cfg.train_list, cfg.dataset_path, augmentator=train_augmentator,
        input_transform=model_cfg.input_transform,
        keep_background_prob=0.05, with_previous=True)
    valset = MyPreviousDataset(
        cfg.val_list,  cfg.dataset_path,
        augmentator=val_augmentator,
        input_transform=model_cfg.input_transform,
